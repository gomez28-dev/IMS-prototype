...
{{--
    Push notification subscribe prompt (Hostinger sync).
    Covers 'accounting' (new order alerts) and 'sales' (clearance updates).
--}}
@php
    $__pushEnabledRoles = ['accounting', 'sales'];
@endphp
@if (Auth::check() && in_array(Auth::user()->role, $__pushEnabledRoles))
<script>
    (function () {
        const VAPID_PUBLIC_KEY = @json(config('services.webpush.vapid_public_key', env('VAPID_PUBLIC_KEY')));
        ...
        if (localStorage.getItem('ims_push_prompted') !== '1') {
            localStorage.setItem('ims_push_prompted', '1');
            subscribeToPush();
